...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename='harness_events' AND policyname='service_role_all'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='harness_webhooks' AND policyname='service_role_all') THEN
    CREATE POLICY "service_role_all" ON harness_webhooks
      FOR ALL TO service_role USING (true) WITH CHECK (true);
  END IF;
END; $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='harness_webhooks' AND policyname='owner_select') THEN
    CREATE POLICY "owner_select" ON harness_webhooks
      FOR SELECT TO authenticated USING (created_by = auth.uid());
  END IF;
END; $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='harness_webhooks' AND policyname='owner_insert') THEN
    CREATE POLICY "owner_insert" ON harness_webhooks
      FOR INSERT TO authenticated WITH CHECK (created_by = auth.uid());
  END IF;
END; $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='harness_webhooks' AND policyname='owner_update') THEN
    CREATE POLICY "owner_update" ON harness_webhooks
      FOR UPDATE TO authenticated USING (created_by = auth.uid()) WITH CHECK (created_by = auth.uid());
  END IF;
END; $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='harness_webhooks' AND policyname='owner_delete') THEN
