...
- **MobileBottomNav role resolution (Issue #92)** — MobileBottomNav no longer fetches role client-side. Role is resolved server-side in DashboardLayout via `getUserRole(user.id)` and passed down as `isAdmin` prop through AppShell. If Admin entry is missing on mobile, check: (1) `fd_user_roles` row has `is_active = true` for the user's actual session user_id (NOT an old/stale user_id — UAT found session user `cec59014` had no row while the admin row was for `b4c20f13`), (2) DashboardLayout is server component (no `'use client'` directive), (3) AppShell receives `isAdmin` prop. Migration `20260314050000_fix_admin_role_session_user_issue92.sql` upserts the correct user_id.
- **`/api/apps` 500 on missing SUPABASE_SERVICE_ROLE_KEY** — The apps API route calls `createSupabaseAdminClient()` to fetch deployment cache. If `SUPABASE_SERVICE_ROLE_KEY` is not set in the Vercel environment, `createSupabaseAdminClient()` may throw, causing the entire route to return 500. Fixed in bugfix #92: wrapped in try-catch so deployment cache gracefully degrades to empty map without crashing the route.
- **Apps page `fd_user_roles` table name** — The apps page client component was querying `dash_user_roles` (wrong table name) for the admin check. Correct table is `fd_user_roles`. Fixed in bugfix #92.
