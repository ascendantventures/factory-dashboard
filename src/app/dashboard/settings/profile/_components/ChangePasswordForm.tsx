...
export function ChangePasswordForm() {
      ...
      <div style={{ position: 'relative' }}>
        <input
          ...
          style={{
            ...
            border: `1px solid ${hasError ? '#EF4444' : '#3F3F46'}`,
            borderRadius: '6px', padding: '0 40px 0 12px',
            fontSize: '14px', color: '#FAFAFA', outline: 'none',
            background: hasError ? 'rgba(239, 68, 68, 0.1)' : '#18181B',
          }}
          onFocus={e => { if (!hasError) { e.currentTarget.style.border = '1px solid #6366F1'; e.currentTarget.style.boxShadow = '0 0 0 3px rgba(99,102,241,0.2)'; }}}
          onBlur={e => { if (!hasError) { e.currentTarget.style.border = '1px solid #3F3F46'; e.currentTarget.style.boxShadow = 'none'; }}}
        />
        <button
          type="button"
          onClick={onToggle}
          style={{
            ...
            border: 'none', background: 'transparent', cursor: 'pointer', color: '#A1A1AA', padding: 0,
          }}
        >
  ...
  return (
    <form onSubmit={handleSubmit} style={{
      ...
      <h3 style={{ fontSize: '18px', fontWeight: 600, color: '#FAFAFA', margin: 0 }}>Change Password</h3>
      ...
      <div>
        ...
        {nextError
          ? <p style={{ fontSize: '12px', color: '#EF4444', fontWeight: 500, marginTop: '6px' }}>{nextError}</p>
          : <p style={{ fontSize: '12px', color: '#71717A', marginTop: '6px' }}>At least 8 characters.</p>
        }
      </div>
      ...
      <div>
        ...
        {confirmError && <p style={{ fontSize: '12px', color: '#EF4444', fontWeight: 500, marginTop: '6px' }}>{confirmError}</p>}
      </div>

      {error && (
        <p data-testid="error-message" style={{ fontSize: '12px', color: '#EF4444', fontWeight: 500 }}>{error}</p>
      )}
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
        <button
          ...
          style={{
            ...
            background: loading || nextError || confirmError ? '#4F46E5' : success ? '#10B981' : '#6366F1',
