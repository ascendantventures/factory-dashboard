...
const FORMAT_OPTIONS: Array<{ value: FormatType; label: string; hint: string; icon: React.ReactNode }> = [
  { value: 'standard', label: 'Standard', hint: 'Generic JSON payload', icon: <Globe size={20} /> },
  { value: 'slack', label: 'Slack', hint: 'Block Kit format for Incoming Webhooks', icon: <Slack size={20} /> },
  { value: 'discord', label: 'Discord', hint: 'Embed format for Discord webhooks', icon: <DiscordIcon size={20} /> },
];
  ...
  return (
    <form onSubmit={handleSubmit} noValidate>
      ...
      <div className="mb-6" style={{ marginTop: '24px' }}>
        <fieldset data-testid="format-selector" role="radiogroup" aria-label="Payload format" style={{ border: 'none', margin: 0, padding: 0 }}>
          <legend style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '14px', fontWeight: 500, color: '#B8BFC7', marginBottom: '12px' }}>
            Payload Format
          </legend>
              ...
              return (
                <label
                  ...
                  style={{
                    ...
                    gap: '12px',
                    padding: '14px 16px',
                    background: isSelected ? 'rgba(99, 102, 241, 0.08)' : '#161A1F',
                    border: `1px solid ${isSelected ? '#6366F1' : '#2E353D'}`,
                    borderRadius: '8px',
                    cursor: 'pointer',
                    transition: 'border-color 150ms ease, background 150ms ease',
                  }}
                  onMouseEnter={(e) => { if (!isSelected) { e.currentTarget.style.borderColor = '#3A424D'; e.currentTarget.style.background = '#1E2329'; } }}
                  onMouseLeave={(e) => { if (!isSelected) { e.currentTarget.style.borderColor = '#2E353D'; e.currentTarget.style.background = '#161A1F'; } }}
                >
                  <input
                    type="radio"
                    name="format_type"
                    value={opt.value}
                    checked={isSelected}
                    onChange={() => setFormatType(opt.value)}
                    aria-label={opt.label}
                    style={{ position: 'absolute', opacity: 0, width: 0, height: 0 }}
                  />
                  <span style={{ color: isSelected ? '#B8BFC7' : '#6B7380', flexShrink: 0 }}>{opt.icon}</span>
                  <div>
                    <div style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '14px', fontWeight: 600, color: '#FAFAFB' }}>{opt.label}</div>
                    <div style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '12px', color: '#6B7380', marginTop: '2px' }}>{opt.hint}</div>
                  </div>
                </label>
              );
            })}
